...
def train_model(model, train_generator, val_generator):
    ...
    checkpoint = ModelCheckpoint(
        MODELS_DIR / 'baseline_processed_best.h5',
    ...
    log_dir = "logs/fit/baseline_processed/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    model.save(MODELS_DIR / 'baseline_processed_final.h5')
...
def evaluate_model(model, test_generator):
    ...
    # Lưu kết quả dự đoán
    with open(RESULTS_DIR / 'baseline_processed_results.txt', 'w') as f:
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / 'baseline_processed_training_history.png')
    plt.close()
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / 'baseline_processed_confusion_matrix.png')
    plt.close()
    ...
    with open(RESULTS_DIR / 'baseline_processed_classification_report.txt', 'w') as f:
        f.write(report)

def main():
    print("===== HUẤN LUYỆN MÔ HÌNH CNN CƠ BẢN VỚI DỮ LIỆU ĐÃ CHUẨN HÓA =====")
    ...
    print("\nĐã hoàn thành huấn luyện và đánh giá mô hình CNN cơ bản với dữ liệu đã chuẩn hóa!")
